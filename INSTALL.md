...
- Put the htick package in the directory where the other packages of fido
  husky reside:
...
```
    https://github.com/huskyproject/htick/releases/latest
```
## Build 
...
the `htick` directory any more. So you may delete them by running
```
    $ make distclean
```
...
`htick` subproject if you did not run `make distclean`.

#### Compiling and installing documentation

You may compile and install the `htick` manual in the following formats:
...
copies in the `htick` directory any more. So you may delete them by running
```
    $ make distclean-doc
```
...
dos:
```sh
   d:\husky\htick>make -f makefile.djg
```
...
You're ready. Now you can install software which uses htick.
