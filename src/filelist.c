...
//  _createDirectoryTree( fileareapath );
  ...
  while( ( file = readdir( dir ) ) != NULL )
  {
    ...
    totalnumber++;
    putFileInFilelist( f, file->d_name, stbuf.st_size, locTime->tm_mday, locTime->tm_mon,
                       locTime->tm_year, tic.anzdesc, tic.desc );
    disposeTic( &tic );
  }
  if( flag )
  {
...
void filelist(  )
{
  ...
  for( i = 0; i < config->fileAreaCount; i++ )
  {
    ...
    {
      printFileArea( config->fileAreas[i].areaName, config->fileAreas[i].pathName,
