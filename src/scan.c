...
void convertMsgText(HMSG SQmsg, s_message * msg, hs_addr ourAka)
{
    ...
    dt = gmtime(&tm);
    sprintf(viaLine,
            "\001Via %u:%u/%u.%u @%04u%02u%02u.%02u%02u%02u.UTC %s",
            ourAka.zone,
            ourAka.net,
            ourAka.node,
            ourAka.point,
...
void scanNMArea(s_area * afixarea)
{
    ...
    netmail = MsgOpenArea((unsigned char *)afixarea->fileName,
                          MSGAREA_NORMAL,
    ...
    if(netmail != NULL)
    {
        ...
        for(i = 1; i <= highMsg; i++)
        {
            ...
            if(findInStrArray(robot->names,
                              (char *)xmsg.to) >= 0 && for_us && (xmsg.attr & MSGREAD) != MSGREAD)
            {
