  ...
  if( area.group )
  {
    if( link->numAccessGrp > 0 )
    {
      for( i = 0; i < link->numAccessGrp; i++ )
        if( strcmp( area.group, link->AccessGrp[i] ) == 0 )
          found = 1;
    }
    ...
    if( config->numPublicGroup > 0 )
    {
      for( i = 0; i < config->numPublicGroup; i++ )
        if( strcmp( area.group, config->PublicGroup[i] ) == 0 )
          found = 1;
    }
  }
  else
...
char *list( s_message * msg, s_link * link )
{
  ...
  xscatprintf( &report,
               "\r '+'  You are receiving files from this area.\r '*'  You can send files to this file echo.\r '&'  You can send and receive files.\r\r%i areas available for %s, %i areas active\r",
               avail, aka2str( link->hisAka ), active );
  if( link->ffixEchoLimit )
    xscatprintf( &report, "\rYour limit is %u areas for subscribe\r", link->ffixEchoLimit );
...
char *linked( s_message * msg, s_link * link, int action )
{
  ...
  if( action == 1 )
    xscatprintf( &report,
                 "\r '+'  You are receiving files from this area.\r '*'  You can send files to this file echo.\r '&'  You can send and receive files.\r\r%u areas linked for %s\r",
                 n, aka2str( link->hisAka ) );
  else
...
char *help( s_link * link )
{
  ...
  if( config->filefixhelp != NULL )
  {
    ...
    {
      w_log( LL_ERR, "FileFix: cannot open help file '%s'", config->filefixhelp );
      return NULL;
    }
...
char *available( s_link * link )
{
  ...
  for( j = 0; j < config->linkCount; j++ )
  {
        ...
        ( ( uplink->LinkGrp == NULL ) || ( found != 0 ) ) )
    {
      ...
      xscatprintf( &report, "Available File List from %s:\r", aka2str( uplink->hisAka ) );
      ...
      w_log( LL_AREAFIX, "Filefix: Available File List from %s sent to %s",
             aka2str( uplink->hisAka ), linkAka );
    }
  }
...
int forwardRequestToLink( char *areatag, char *descr, s_link * uplink, s_link * dwlink, int act )
{
  s_message *msg;
...
static int compare_links_priority( const void *a, const void *b )
{
  ...
  if( config->links[ia].forwardFilePriority < config->links[ib].forwardFilePriority )
    return -1;
  else if( config->links[ia].forwardFilePriority > config->links[ib].forwardFilePriority )
    return 1;
  else
    return 0;
}
...
int forwardRequest( char *areatag, s_link * dwlink )
{
  ...
  {
    if( config->links[i].forwardFileRequests )
...
char *subscribe( s_link * link, s_message * msg, char *cmd )
{
  ...
  for( i = 0; i < config->fileAreaCount; i++ )
  {
    ...
    case 0:
      ...
      w_log( LL_AREAFIX, "FileFix: %s already linked to %s", aka2str( link->hisAka ),
             area->areaName );
      break;
    case 1:
    ...
    case 5:
      ...
      w_log( LL_AREAFIX, "FileFix: area %s -- link is not possible for %s", area->areaName,
             aka2str( link->hisAka ) );
      break;
    default:
      xscatprintf( &report, "%s No access\r", area->areaName );
      w_log( LL_AREAFIX, "FileFix: filearea %s -- no access for %s", area->areaName,
             aka2str( link->hisAka ) );
      continue;
    }
  }
...
char *unsubscribe( s_link * link, s_message * msg, char *cmd )
{
  ...
  for( i = 0; i < config->fileAreaCount; i++ )
  {
    ...
    case 5:
      ...
      w_log( '8', "FileFix: area %s -- unlink is not possible for %s", area->areaName,
             aka2str( link->hisAka ) );
      break;
    default:
      w_log( '8', "FileFix: area %s -- no access for %s", area->areaName, aka2str( link->hisAka ) );
      continue;
    }
  }
  if( !report )
...
char *processcmd( s_link * link, s_message * msg, char *line, int cmd )
{
  ...
  case ADD:
    ...
    break;
  case DEL:
    report = unsubscribe( link, msg, line );
    RetFix = DEL;
    break;
  case UNLINK:
    report = unlinked( msg, link );
    RetFix = UNLINK;
    break;
  case LINKED:
    report = linked( msg, link, 1 );
    RetFix = LINKED;
    break;
  case AVAIL:
    ...
    break;
  case PAUSE:
    report = pause_link( msg, link );
    RetFix = PAUSE;
    break;
  case RESUME:
    report = resume_link( msg, link );
    RetFix = RESUME;
    break;
  ...
  while( ptmp )
  {
    ...
    *report = 0;
    ...
    if( strlen( tmp ) > 50 )
      tmp[50] = 0;
    if( 50 - strlen( tmp ) == 0 )
      sprintf( tmpBuff, " %s  %s\r", tmp, report );
    else if( 50 - strlen( tmp ) == 1 )
      sprintf( tmpBuff, " %s   %s\r", tmp, report );
    else
      sprintf( tmpBuff, " %s %s  %s\r", tmp, print_ch( 50 - strlen( tmp ) - 1, '.' ), report );
...
int processFileFix( s_message * msg )
{
  ...
  if( link != NULL )
  {
    if( link->FileFix == 1 )
    {
      ...
      {
        if( stricmp( link->fileFixPwd, msg->subjectLine ) == 0 )
          security = 0;
        else
          security = 3;
      }
...
int autoCreate( char *c_area, char *descr, ps_addr pktOrigAddr, ps_addr dwLink )
{
  ...
  bDir = ( creatingLink->fileBaseDir ) ? creatingLink->fileBaseDir : config->fileAreaBaseDir;
  ...
  fileName =
      creatingLink->autoFileCreateFile ? creatingLink->autoFileCreateFile : getConfigFileName(  );
  ...
  if( creatingLink->LinkGrp &&
      !( creatingLink->autoFileCreateDefaults
         && fc_stristr( creatingLink->autoFileCreateDefaults, "-g " ) ) )
  {
  ...
  if( config->ReportTo && !cmAnnNewFileecho && ( area = getFileArea( c_area ) ) != NULL )
  {
    if( getNetMailArea( config, config->ReportTo ) != NULL )
    {
      msg = makeMessage( area->useAka,
                         ...
                         config->sysop, "Created new fileareas", 1, config->filefixReportsAttr );
    ...
    else
    {
      msg = makeMessage( area->useAka,
                         area->useAka,
                         config->filefixFromName ? config->filefixFromName : versionStr,
                         "All", "Created new fileareas", 0, config->filefixReportsAttr );
