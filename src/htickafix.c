...
int checkAccessAndOptGrps(s_area * echo, s_link * link, int imp_exp)
{
    ...
    {
        /* check group access */
        if(!grpInArray(echo->group, config->PublicGroup,
                       config->numPublicGroup) &&
...
int processFileFix(s_message * msg)
{
    ...
    if(!security)
    {
        ...
        while(token != NULL)
        {
            preport = filefix_processcmd(link,
                                         msg,
                                         stripLeadingChars(token, " \t"),
    ...
    if(cmd)
    {
        ...
        if(link)
        {
            tmpmsg = makeMessage(&addr,
                                 ...
                                 link->filefix.pwd ? link->filefix.pwd : "",
                                 1,
