...
int processHatchParams(int i, int argc, char ** argv)
{
    ...
    if(stricmp(argv[i], "replace") == 0)
    {
        ...
        {
            basename = strrchr(hatchInfo->file, PATH_DELIM) ? strrchr(hatchInfo->file,
                                                                      PATH_DELIM) +
                       1 : hatchInfo->file;
...
void processConfig()
{
    ...
    htick_log = openLog(LogFileName, versionStr);       /* if failed: openLog() prints a message
                                                           to stderr */
    ...
    {
        w_log(LL_CRIT,
              "Parameter MaxTicLineLength (%d) in fidoconfig must be 0 or >80\n",
